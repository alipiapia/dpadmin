...
class Brand extends Admin
{
    ...
    public function index()
    {
        ...
        return ZBuilder::make('table')
            ->setPageTitle('品牌管理') // 设置页面标题
            ->setTableName('Brand') // 设置数据表名
            ->setSearch(['id' => 'ID', 'name' => '品牌名称']) // 设置搜索参数
            ->addColumns([ // 批量添加列
                ['id', 'ID'],
                ['name', '名称', 'text.edit'],
                ['desc', '描述', 'text.edit'],
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($brand = BrandModel::create($data)) {
                // 记录行为
                action_log('brand_add', 'brand', $brand['id'], UID);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($brand = BrandModel::update($data)) {
                // 记录行为
                action_log('brand_edit', 'brand', $brand['id'], UID, get_nickname($brand['id']));
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        return parent::setStatus($type, ['brand_'.$type, 'brand', $uid_delete, UID, implode('、', $ids)]);
    }
    ...
    public function quickEdit($record = [])
    {
        ...
        $config  = BrandModel::where('id', $id)->value($field);
        $details = '字段(' . $field . ')，原值(' . $config . ')，新值：(' . $value . ')';
        return parent::quickEdit(['brand_edit', 'brand', $id, UID, $details]);
    }
}
